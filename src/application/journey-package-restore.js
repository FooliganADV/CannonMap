...
export async function verifyRestoredDayPayload(payload,restored){
  ...
  const byFeatureId=new Map(features.map(item=>[String(item.id),item]));for(const expectedFeature of expectedFeatures){const feature=byFeatureId.get(String(expectedFeature.id));if(!feature||feature.status!==expectedFeature.status||Number(feature.scoreAwarded||0)!==Number(expectedFeature.scoreAwarded||0)||feature.type!==expectedFeature.type)throw new Error(`Restore verification failed: objective state mismatch for ${expectedFeature.id}.`);}
  ...
  const expectedById=new Map(payload.media.map(item=>[String(item.mediaId),item])),names=new Set(),pairs=new Map();for(const record of restored.media){const expectedRecord=expectedById.get(String(record.mediaId));if(!expectedRecord||record.name!==expectedRecord.name||record.role!==expectedRecord.role||record.cameraRole!==expectedRecord.cameraRole||record.pairId!==expectedRecord.pairId||record.pairedMediaId!==expectedRecord.pairedMediaId||names.has(record.name))throw new Error('Restore verification failed: media identity, filename, or relationship mismatch.');names.add(record.name);const bytes=new Uint8Array(await record.blob.arrayBuffer());if(!bytes.byteLength||await digest(bytes)!==expectedRecord.checksum.value)throw new Error(`Restore verification failed: checksum mismatch for ${record.name}.`);if(record.pairId){if(!pairs.has(record.pairId))pairs.set(record.pairId,[]);pairs.get(record.pairId).push(record);}}
  for(const [pairId,records] of pairs)if(records.length!==4||!['front','rear'].every(cameraRole=>['original','evidence'].every(role=>records.some(record=>record.cameraRole===cameraRole&&record.role===role))))throw new Error(`Restore verification failed: incomplete Capture Pair ${pairId}.`);
  return Object.freeze({verified:true,projectId:project.projectId,projectName:project.name,dayNumber:day,status:manifest.dayState?.status||project.rallyExecution?.days?.[day]?.status||null,...actual,journalEventCount:restored.journal.length,pairCount:pairs.size,mediaCount:restored.media.length,filenames:[...names]});
}
