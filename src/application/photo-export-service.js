...
export function createPhotoExportService({repository}={}){
  ...
  return Object.freeze({
    ...
    async dayBackup(projectId,dayNumber,{journal=[],project=null,settings={},applicationVersion=null,buildId=null}={}){
      ...
      const dayFeatures=durable((project.features||[]).filter(item=>Number(item.day)===day)),durableProject=durable(project),createdAt=new Date().toISOString(),projectMetadata={projectId:String(projectId),projectName:project.name||null,executionId:project.executionId||project.rallyExecution?.executionId||null,finalizedMasterId:project.finalizedMasterId||project.sourceMasterId||null,dayNumber:day,project:durableProject,dayFeatures,settings:durable(settings),createdAt};
