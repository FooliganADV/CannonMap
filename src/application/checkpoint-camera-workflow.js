...
export function createCheckpointCameraWorkflow({mediaRepository,photoEvidence,journal,clock,createId,onState=()=>{},onRequested=()=>{},onDiagnostic=()=>{}}={}){
  ...
  async function recordFailure(error,role,status='write_failed'){
    ...
    const sessionId=active.evidenceContext?.sessionId||null;
    try{await journal.appendEventIdempotent({eventId:`${active.pairJournalEventId}:storage:${role}:${status}`,projectId:active.projectId,sessionId,eventType:'media_storage_failure',source:'checkpoint_camera',title:`Photo pair storage failed · ${active.checkpoint.name}`,summary:`${role.toUpperCase()} capture did not complete durable pair persistence.`,references:{checkpointId:active.checkpoint.id,parentEventId:active.journalEvent.eventId,pairId:active.pairId,sessionId},metadata:{pairId:active.pairId,cameraRole:role,status,retryable:true,sessionId,sessionRunNumber:active.evidenceContext?.sessionRunNumber??null,sessionCalendarDate:active.evidenceContext?.sessionCalendarDate??null}});}catch(_){ }
  }
  async function finalizePair(){
    if(!active?.sides.front||!active?.sides.rear)return null;
    active.status='saving_pair';publish();
    const front=active.sides.front,rear=active.sides.rear,sessionId=active.evidenceContext?.sessionId||null,event=await journal.appendEventIdempotent({
      eventId:active.pairJournalEventId,projectId:active.projectId,sessionId,eventType:'photo_added',source:'checkpoint_camera',title:`Capture Pair · ${active.checkpoint.name}`,
      ...
      metadata:{pairId:active.pairId,persistenceStatus:'complete',checkpointId:active.checkpoint.id,objectiveType:active.checkpoint.type||'checkpoint',dayNumber:Number(active.checkpoint.day)||null,projectId:active.projectId,sessionId:active.evidenceContext?.sessionId||null,sessionRunNumber:active.evidenceContext?.sessionRunNumber??null,sessionCalendarDate:active.evidenceContext?.sessionCalendarDate??null,sessionStartTimestamp:active.evidenceContext?.sessionStartedAt??null,frontCapturedAt:front.metadata.captureTimestamp||front.metadata.capturedAt,rearCapturedAt:rear.metadata.captureTimestamp||rear.metadata.capturedAt}
    });
