...
const TOP_LEVEL_KEYS=new Set(['schemaVersion','algorithmVersion','eventId','observationId','riderId','checkpointId','deviceSessionId','sequence','occurredAt','createdAt','updatedAt','captureSource','lifecycleState','syncState','nextAttemptAt','observed','derived','evidenceRefs']);
const OBSERVED_KEYS=new Set(['timestampMs','location','accuracyMeters','altitudeMeters','altitudeAccuracyMeters','headingDegrees','speedMetersPerSecond']);
...
const expectedIdempotencyKey=observation=>`observation:${observation?.eventId||''}:${observation?.observationId||''}`;
...
export function validateObservationIngress(observation,{nowMs=Date.now(),idempotencyKey}={}){
  ...
  if(observation.schemaVersion!==1)errors.push('unsupported schemaVersion');
  ...
  for(const key of ['eventId','observationId','riderId','deviceSessionId'])if(typeof observation[key]!=='string'||!IDENTIFIER.test(observation[key]))errors.push(`${key} is invalid`);
  ...
  if(!Number.isSafeInteger(observation.sequence)||observation.sequence<0)errors.push('sequence is invalid');
  ...
  if(idempotencyKey!==expectedIdempotencyKey(observation))errors.push('idempotency key does not match observation');
