...
test('a later complete four-asset pair supersedes stale failed attempts idempotently',async()=>{const failed=[media('original','front',{pairId:'pair-failed',suffix:'-failed',pairStatus:'abandoned'})],complete=['front','rear'].flatMap(role=>[media('original',role,{pairId:'pair-success',suffix:'-success',capturedAt:'2026-08-21T15:02:00.000Z'}),media('evidence',role,{pairId:'pair-success',suffix:'-success',capturedAt:'2026-08-21T15:02:01.000Z'})]),records=[...failed,...complete],events=[arrival(),{eventId:'old-failure',eventType:'photo_failed',timestamp:'2026-08-21T15:00:00.000Z',references:{checkpointId:'cp-1',pairId:'pair-failed'}},{eventId:'success-event',eventType:'photo_added',timestamp:'2026-08-21T15:02:02.000Z',references:{checkpointId:'cp-1',pairId:'pair-success'}}],item=checkpoint({pendingPhotoPair:{pairId:'pair-failed',pairJournalEventId:'old-failure',status:'failed'}}),service=createCheckpointEvidenceReconciliationService({mediaRepository:repository(records)});const first=await service.inspect({projectId:'project',checkpoint:item,journalEvents:events}),second=await service.inspect({projectId:'project',checkpoint:item,journalEvents:events});assert.equal(first.pairId,'pair-success');assert.equal(first.action,'finalize_pair');assert.equal(first.photoEvidenceState,'complete');assert.deepEqual(first.mediaIds,second.mediaIds);assert.equal(records.length,5);});
