...
const media=(id,index=0)=>({mediaId:id,projectId:'project-1',sessionId:'session-1',role:'original',blob:new Blob([`photo-${index}`]),metadata:{sessionId:'session-1',dayNumber:1}});
...
function harness({saveFailure=null,external=null}={}){
  ...
  const mediaRepository={async listProjectPhotos(){mediaQueries.project++;return mediaRows;},async listProjectSessionPhotos(projectId,sessionId){mediaQueries.session++;return mediaRows.filter(item=>item.projectId===projectId&&item.sessionId===sessionId);},async getMedia(id){return mediaRows.find(item=>item.mediaId===id)||null;}};
