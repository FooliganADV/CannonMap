...
test('v10 upgrade retains legacy v9 mediaRecords without relabeling or copying them',async({page},testInfo)=>{
  ...
  expect(result.version).toBe(12);expect(result.stores).toContain('mediaRecords');expect(result.stores).toContain('missionMedia');expect(result.stores).toContain('recoverySnapshots');expect(result.stores).toContain('backupDirectoryHandles');
