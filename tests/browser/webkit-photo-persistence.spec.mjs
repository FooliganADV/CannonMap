...
test('WebKit restores PHOTO_REQUIRED after termination and permits durable retry',async({page,context})=>{
  ...
  await page.evaluate(()=>window.CannonMapTest.completeCurrentCheckpoint(true));await expect(page.locator('#rallyCameraWorkflow')).toBeVisible();await expect(page.locator('#rallyCompleteButton')).toBeDisabled();await page.close();
  const restored=await context.newPage();await restored.goto('/?e2e=webkit-camera-retry');await restored.waitForFunction(()=>document.documentElement.dataset.cannonmapReady==='true');await expect(restored.locator('#rallyCameraWorkflow')).toBeVisible();await expect(restored.locator('#rallyCompleteButton')).toBeDisabled();
...
test('WebKit restores a front-only partial pair and resumes the missing rear side',async({page,context})=>{
  ...
  await expect(page.locator('#rallyCameraPhotoCount')).toContainText('Front captured ✓');await expect(page.locator('#rallyCameraPhotoCount')).toContainText('Rear required');await expect(page.locator('#rallyCameraCapturePair')).toHaveText('RESUME PAIR');await expect(page.locator('#rallyCompleteButton')).toBeDisabled();
