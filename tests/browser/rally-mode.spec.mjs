...
test('Mission Control exposes only the one-tap paired-photo workflow',async({page},testInfo)=>{
  test.skip(testInfo.project.name==='desktop');
  await page.addInitScript(()=>{globalThis.__CANNONMAP_FEATURE_FLAGS__={'architecture.journal.checkpoint-photos':true};});
  await loadProject(page);
  await page.locator('#rallyMoreButton').click();
  await expect(page.locator('#rallyPhotoCapturePair')).toBeVisible();
  await expect(page.locator('#rallyPhotoCapturePair')).toHaveText('CAPTURE PAIR');
  await expect(page.locator('#rallyCameraWorkflow,#rallyCameraInput')).toHaveCount(0);
  await expect(page.locator('#rallyPhotoDebug')).toBeHidden();
  await expect(page.locator('#rallyPhotoFrontButton,#rallyPhotoRearButton,#rallyPhotoSubmitPair')).toHaveCount(3);
});
