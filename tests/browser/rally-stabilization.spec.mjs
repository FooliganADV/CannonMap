...
test('photo evidence survives reload, opens in the viewer, and exports stable filenames',async({page,context},testInfo)=>{
  ...
  await page.reload();await page.waitForFunction(()=>document.documentElement.dataset.cannonmapReady==='true');await page.locator('#rallyMoreButton').click();await expect(page.locator('#rallyPhotoViewerButton')).toBeVisible();await page.locator('#rallyPhotoViewerButton').click();await expect(page.locator('.rally-photo-group')).toHaveCount(1);await expect(page.locator('.rally-photo-card')).toHaveCount(2);await expect(page.locator('.rally-photo-storage-warning')).toContainText('stored locally on this device');await page.screenshot({path:testInfo.outputPath('checkpoint-photos-gallery.png')});await page.locator('.rally-photo-card').first().click();
...
test('deferred resume and finish, hotel completion, reload, and explicit Day 2 start are durable',async({page},testInfo)=>{
  ...
  await expect(page.locator('#rallyDayCompleteTitle')).toHaveText('✓ Day Complete');await expect(page.locator('#rallyDayBackupStatus')).toHaveText('Not backed up');await expect(page.locator('#rallyBackupToday')).toHaveText('Back Up Today');
  ...
  await page.locator('#rallyBackupToday').click();await expect(page.locator('#rallyBackupSheet')).toBeVisible();await expect(page.locator('#rallyBackupSheetTitle')).toHaveText('Back Up Today’s Photos');await expect(page.locator('#rallyBackupSheet')).not.toContainText(/â|Ã|Â|�/);
