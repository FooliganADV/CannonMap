...
test('finalized Day 9 exports verified photos, Journal, restorable backup, and Journey manifest from an explicitly selected active day',async({page},testInfo)=>{
  ...
  pending=page.waitForEvent('download');await page.locator('#rallyBackupDayPackage').click();const backup=await pending,backupBytes=await readFile(await backup.path()),restorable=await readStoredDayPackage(new Blob([backupBytes]));expect(backup.suggestedFilename()).toBe('Day09_Backup.cmapday.zip');expect(restorable.manifest.mediaCount).toBe(8);expect(restorable.journal.length).toBeGreaterThan(4);expect(restorable.media.filter(item=>item.metadata.objectiveType==='hotel')).toHaveLength(4);const stages=await page.evaluate(()=>window.CannonMapTest.rallyDebugEntries().map(entry=>entry.type).filter(type=>type.startsWith('backup_')));expect(stages).toEqual(expect.arrayContaining(['backup_started','backup_project_resolved','backup_day_resolved','backup_journal_loaded','backup_media_loaded','backup_manifest_created','backup_zip_created','backup_verified','backup_download_requested','backup_completed']));
