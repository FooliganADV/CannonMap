...
test('finalized Day 9 exports verified photos, Journal, restorable backup, and Journey manifest from an explicitly selected active day',async({page},testInfo)=>{
  ...
  pending=page.waitForEvent('download');await page.locator('#rallyBackupDayPackage').click();const backup=await pending,backupBytes=await readFile(await backup.path()),restorable=await readStoredDayPackage(new Blob([backupBytes]));expect(backup.suggestedFilename()).toMatch(/^CannonMap_FieldExportDay9_D09_Run01_\d{4}-\d{2}-\d{2}_\d{6}-\d{3}_Backup\.cmapday\.zip$/);expect(restorable.manifest.mediaCount).toBe(8);expect(restorable.journal.length).toBeGreaterThan(4);expect(restorable.media.filter(item=>item.metadata.objectiveType==='hotel')).toHaveLength(4);await expect(page.locator('#rallyBackupSheetStatus')).toContainText(/package verified/i);await expect(page.locator('#rallyBackupSheetStatus')).toContainText(/confirm.*download/i);await expect(page.locator('#rallyBackupSheetStatus')).not.toContainText(/Backup complete|package exported/i);const stages=await page.evaluate(()=>window.CannonMapTest.rallyDebugEntries().map(entry=>entry.type).filter(type=>type.startsWith('backup_')));expect(stages).toEqual(expect.arrayContaining(['backup_started','backup_project_resolved','backup_day_resolved','backup_journal_loaded','backup_media_indexed','backup_manifest_created','backup_zip_created','backup_verified','backup_download_requested','backup_package_verified_download_requested']));expect(stages).not.toContain('backup_completed');
