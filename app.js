...
function sanitizeProjectData(project,source='project import'){
  ...
  safe.features=filterProhibitedFeatures(safe.features,source).map((feature,index)=>normalizeCheckpoint(feature,index));
...
function parseGpx(xmlText,filename) {
  ...
  [...doc.getElementsByTagName('wpt')].forEach((wpt,index)=>{
    ...
    if(validPoint(p)){
      ...
      const feature={id:uid(),name,type,day:inferDay(`${name} ${notes} ${filename}`),assignmentMethod:'',notes,visible:true,source:filename,sourceOrder:sourceOrder++,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),geometry:{kind:'point',coordinates:[p]}};
...
function startGps() {
  ...
  state.gpsWatchId=navigator.geolocation.watchPosition(position=>{
    const ll=[position.coords.latitude,position.coords.longitude];state.lastGpsPosition={lat:ll[0],lon:ll[1],time:new Date(position.timestamp||Date.now()).toISOString()};state.gpsLayer?.remove();state.gpsAccuracyLayer?.remove();
...
function renderRallyMode(){
  ...
  $('rallyDeferButton').disabled=!next;$('rallyCompleteButton').disabled=!next;$('rallySkipButton').disabled=!next;$('rallyRestoreButton').disabled=!dayCheckpoints().some(feature=>feature.status==='deferred');$('goHotelButton').disabled=!hotel.hotel&&!state.hotelBailoutActive;$('goHotelButton').textContent=state.hotelBailoutActive?'UNDO HOTEL BAILOUT':'GO TO HOTEL';
}
...
function completeCurrentCheckpoint(){const checkpoint=currentCheckpoint();if(!checkpoint)return setStatus('No active checkpoint.',true);snapshot();checkpoint.status='completed';checkpoint.completedAt=new Date().toISOString();checkpoint.deferredAt=null;checkpoint.deferReason=null;saveProject(false);renderAll();setStatus(`Completed ${checkpoint.name} for ${checkpoint.points} points.`);}
function deferCurrentCheckpoint(reason='Rider deferred'){const checkpoint=currentCheckpoint();if(!checkpoint)return setStatus('No active checkpoint.',true);snapshot();checkpoint.status='deferred';checkpoint.deferredAt=new Date().toISOString();checkpoint.deferReason=reason;saveProject(false);renderAll();setStatus(`Deferred ${checkpoint.name}; it remains in the daily sequence.`);}
function restoreDeferredCheckpoint(){const checkpoint=dayCheckpoints().find(feature=>feature.status==='deferred');if(!checkpoint)return setStatus('No deferred checkpoint to restore.',true);snapshot();checkpoint.status='planned';checkpoint.restoredAt=new Date().toISOString();saveProject(false);renderAll();setStatus(`Restored ${checkpoint.name}.`);}
function skipCurrentCheckpoint(){const checkpoint=currentCheckpoint();if(!checkpoint)return setStatus('No active checkpoint.',true);if(!confirm(`Skip ${checkpoint.name}? It will remain in the project as skipped.`))return;snapshot();checkpoint.status='skipped';saveProject(false);renderAll();setStatus(`Skipped ${checkpoint.name}.`);}
...
window.CannonMapTest={filterProhibitedFeatures,sanitizeProjectData,lineGeometriesMatch,lineDistanceMiles,planningMileage,normalizeCheckpoint,selectNextCheckpoint,completeCurrentCheckpoint,deferCurrentCheckpoint,restoreDeferredCheckpoint,skipCurrentCheckpoint,goToHotel,rallyScore,restoreSnapshot};
init();
